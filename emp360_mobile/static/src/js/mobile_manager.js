/** @odoo-module **/
/**
 * EMPLOYEE 360 — MOBILE MANAGER / TEAM SCREEN  v2.0
 */
...
export class MobileManager extends Component {
    ...
    setup() {
        ...
        this.state = useState({
            ...
            showEmpDetail: false, empDetail: null, empDetailBusy: false,
            teamSummary: { total: 0, checked_in: 0, beat_active: 0, visits_today: 0, orders_today: 0, sales_today: 0 },
        });
    ...
    async _load() {
        ...
        try {
            ...
            if (!empIds.length) { this.state.loading = false; return; }
            ...
            const [attRows, beatRows, visitRows] = await Promise.all([
                this.orm.searchRead("hr.attendance",
                    ...
                    ["id", "employee_id", "check_in", "check_out", "worked_hours"], { limit: 500 }),
                this.orm.searchRead("beat.module",
                    [["employee_id", "in", empIds], ["beat_date", "=", today]],
                    ["id", "employee_id", "status", "name", "customer_count"], { limit: 200 }),
                this.orm.searchRead("visit.model",
                    [["employee_id", "in", empIds], ["actual_start_time", ">=", today + " 00:00:00"]],
                    ["id", "employee_id", "status", "total_order_amount", "order_count"], { limit: 500 }),
            ]);
            ...
            const sv = Object.values(stats);
            this.state.teamSummary = {
                total:        employees.length,
                checked_in:   sv.filter(s => s.checked_in).length,
                beat_active:  sv.filter(s => s.beat_status === "in_progress").length,
                visits_today: sv.reduce((s, e) => s + e.visits_done, 0),
                orders_today: sv.reduce((s, e) => s + e.orders_count, 0),
                sales_today:  sv.reduce((s, e) => s + e.sales, 0),
            };
            this._applyFilters();
        } catch (e) { console.error("[MobileManager]", e); }
        finally     { this.state.loading = false; }
    }

    _applyFilters() {
        let list = [...this.state.employees];
        if (this.state.filter === "active")   list = list.filter(e => this.state.empStats[e.id]?.checked_in);
        if (this.state.filter === "inactive") list = list.filter(e => !this.state.empStats[e.id]?.checked_in);
    ...
    setFilter(f)      { this.state.filter = f; this._applyFilters(); }
    ...
    async openEmpDetail(emp) {
        ...
        try {
            ...
            const [empRows, monthVisits, monthColls] = await Promise.all([
                this.orm.searchRead("hr.employee", [["id", "=", emp.id]],
                    ["name", "work_email", "work_phone", "department_id", "job_id"], { limit: 1 }),
                this.orm.searchRead("visit.model",
                    ...
                    { limit: 100 }),
                this.orm.searchRead("visit.collection",
                    [["employee_id", "=", emp.id], ["date", ">=", firstDay], ["state", "=", "confirmed"]],
                    ["amount"], { limit: 200 }),
            ]);
            ...
            this.state.empDetail = {
                ...
                month: {
                    ...
                    sales:        done.reduce((s, v) => s + (v.total_order_amount || 0), 0),
                    collections:  monthColls.reduce((s, c) => s + (c.amount || 0), 0),
                },
                todayVisits: monthVisits.filter(v => v.actual_start_time?.startsWith(today)).slice(0, 6),
            };
        } catch (e) { console.error("[EmpDetail]", e); }
        finally     { this.state.empDetailBusy = false; }
    }

    closeEmpDetail() { this.state.showEmpDetail = false; this.state.empDetail = null; }
    ...
    getAvatarGradient(str) {
        const g = ["linear-gradient(135deg,#4361ee,#3a0ca3)","linear-gradient(135deg,#06d6a0,#019b72)","linear-gradient(135deg,#f72585,#b5179e)","linear-gradient(135deg,#f77f00,#d62828)","linear-gradient(135deg,#7209b7,#3a0ca3)","linear-gradient(135deg,#4cc9f0,#0077b6)"];
        let h = 0;
    ...
    beatStatusClass(s) {
        return ({ in_progress: "warning", completed: "success", pending: "info", draft: "muted" })[s] || "muted";
    }
